  ...
  Papa.parse(file, {
    ...
    complete: res => {
      ...
      if(!mapping.funcionario || !mapping.id_pacote){
        document.getElementById('uploadStatus').innerHTML = `<p style="color:var(--err);font-size:12.5px;">Não consegui identificar as colunas de Funcionário e/ou ID do Pacote nesse CSV. Colunas encontradas: ${headers.join(', ')}.</p>`;
        return;
      }
      importData(res.data, mapping, file.name);
    },
    error: err => {
