  ...
  <main>
    ...
    <section class="view active" id="view-upload">
      ...
      <label class="dropzone" id="dropzone">
        ...
        <div class="big">📄 Clique ou arraste o arquivo CSV aqui</div>
      ...
      <div class="btn-row" style="margin-top:20px;">
        <button class="btn secondary" id="btnMapColumns" style="display:none;">Confirmar mapeamento de colunas</button>
      </div>
...
let pendingParsed = null; // dados aguardando confirmação de mapeamento
...
document.querySelectorAll('.menu-item').forEach(item=>{
  item.addEventListener('click', ()=>{
    ...
    if(item.dataset.view === 'resumo') renderResumo();
    if(item.dataset.view === 'funcionarios') renderFuncionarios();
    if(item.dataset.view === 'historico') renderHistorico();
  });
});
...
function handleFile(file){
  ...
  Papa.parse(file, {
    ...
    complete: res => {
      ...
      pendingParsed = { data: res.data, headers, mapping, filename: file.name };
...
function showMappingUI(headers, mapping){
  const fields = [
    ['funcionario','Funcionário'],['rota','Rota'],['id_pacote','ID do Pacote'],
    ['horario','Horário'],['status','Status (correto/incorreto)']
  ];
  let html = `<div class="card" style="margin-top:14px;"><b style="font-size:12.5px;">Confirme o mapeamento de colunas:</b>
  <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(220px,1fr));margin-top:12px;">`;
  fields.forEach(([key,label])=>{
    html += `<div>
      <label style="font-size:11px;color:var(--text-dim);display:block;margin-bottom:4px;">${label}</label>
      <select id="map_${key}" style="width:100%;background:var(--bg-panel2);border:1px solid var(--border);color:var(--text);padding:8px;border-radius:6px;">
        <option value="">-- não usar --</option>
        ${headers.map(h=>`<option value="${h}" ${h===mapping[key]?'selected':''}>${h}</option>`).join('')}
      </select>
    </div>`;
  });
  html += `</div></div>`;
  document.getElementById('mapArea').innerHTML = html;
  document.getElementById('btnMapColumns').style.display = 'inline-block';
  document.getElementById('uploadStatus').innerHTML = `<p style="color:var(--text-dim);font-size:12.5px;">${pendingParsed.data.length} linhas encontradas em "${pendingParsed.filename}".</p>`;
}

document.getElementById('btnMapColumns').addEventListener('click', ()=>{
  if(!pendingParsed) return;
  const mapping = {
    funcionario: document.getElementById('map_funcionario').value,
    rota: document.getElementById('map_rota').value,
    id_pacote: document.getElementById('map_id_pacote').value,
    horario: document.getElementById('map_horario').value,
    status: document.getElementById('map_status').value
  };
  if(!mapping.funcionario || !mapping.id_pacote){
    toast('Selecione pelo menos as colunas Funcionário e ID do Pacote.');
    return;
  }
  importData(pendingParsed.data, mapping, pendingParsed.filename);
});
...
function importData(rows, mapping, filename){
  ...
  document.getElementById('btnMapColumns').style.display = 'none';
  pendingParsed = null;
  fileInput.value = '';
  toast(`Importação concluída: ${count} linhas.`);
  refreshAllViews();
}
